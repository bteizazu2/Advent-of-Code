def checkRepeatMasks(number):
    """
    Finds repeating sequences in a 
    """
    ...
    for i in range(1, number):
        if number % i == 0: 
            '''
            print(i)
            mask = str(10**(i-1))*(number//i-1)+"1"
            print(mask)
            '''
            duplicate_masks.append(str(10**(i-1))*(number//i-1)+"1")
...
for someRange in (ranges.split(",")):
    ...
    for i in range(len(bound[0]), len(bound[1])+1):
        ...
        print(f"\tDigits: {len(sub_bound[0])}")
        ...
        if len(sub_bound[0]) != len(sub_bound[1]):
            ...
            repeat_masks = checkRepeatMasks(digits)
...
REAL_invalid_IDs = list(dict.fromkeys(invalid_IDs))
